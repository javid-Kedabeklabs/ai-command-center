...
Task packets, ownership conflict detection, result/model provenance, capacity handling, atomic task persistence, owned-process cancellation, isolated worktree inspection, and explicit integration are deterministic modules. Worktree leases persist atomically and reconcile fail-closed after restart; unknown, dirty, mismatched, and unintegrated work is preserved for review. The local Collaboration API can validate and store a packet but cannot dispatch it, and the UI explicitly reports that automatic integration remains disabled. The standard release verifier uses fixtures only and consumes no Claude capacity.
