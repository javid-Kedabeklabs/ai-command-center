...
| Complete UX | VERIFIED_FOR_CURRENT_SCOPE | Shared four-mode canvas, truthful context actions, command palette, copy/paste/duplicate/delete/undo/search/Escape keyboard journey, smart insertion, recovery cards, technical panels, reduced-motion CSS, 1280px containment, macOS visual baseline, and ten live Playwright/Axe journeys | Direct Python diff generation and edge data monitoring remain visibly disabled with reasons rather than fake controls |
...
| Agent primitives | IN_PROGRESS | Schemas, role cards, resolution and migration preview | Persistence/runtime/UI migration remains product breadth work |
