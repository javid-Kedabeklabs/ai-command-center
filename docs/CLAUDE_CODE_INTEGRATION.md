...
Accepted-delivery learning is observation-only. The atomic local metrics store derives ready/start/handoff timing from durable task receipts, accepts an owner-recorded outcome only after terminal task evidence, requires an integrated or rejected lease disposition for modifying work, and records later escaped defects append-only. Exact command replay is idempotent and conflicts fail closed. Fable eligibility requires at least twelve accepted comparable observations with twelve baseline estimates, at least 80% first-pass acceptance, at least 15% lower median calendar lead, zero boundary violations, and zero attributable S1/S2 defects. Qwen preflight eligibility requires twenty measured reviews, at least 25% accepted findings, and zero false blockers. These outputs are always shadow recommendations; they cannot dispatch, integrate, alter policy, or weaken tests.
