...
Packet hashes are not entered manually. `npm run collaboration:prepare < task-draft.json` reads a bounded draft, resolves exact committed Git blobs at the current reviewed HEAD, automatically adds authority files to read-only context, and returns the validated packet with a nondispatch receipt. Dirty source, stale bases, absent blobs, writable authority overlap, missing categories, and scenario/acceptance gaps fail closed. The command has no queue, dispatch, signing, network, or repository-write authority.
