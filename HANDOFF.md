...
- Current focused evidence: schema 9/9, ports 5/5, comments 10/10, component manifests 10/10, scheduler 5/5, per-run resources 5/5, process-global model scheduling 8/8, reusable components 9/9, runtime recovery 36/36, MCP registry/client 12/12 plus live transports 4/4, Keychain secret registry/API/runtime/live 4/4 + 5/5 + 5/5 + 5/5, provider environment 1/1, durable trigger store 17/17, deterministic schedules 13/13, live trigger API 10/10, folder foundation 9/9, folder service 2/2, real folder two-restart recovery 5/5, smoke 30/30, atomic persistence 6/6, and multi-trigger manager helpers/API 6/6. The integrated browser/Axe gate covers all four complexity modes and every persisted trigger state and passed 90/90 repeated runs. Phase 5 is verified. Exact lifecycle, request boundary, redaction, Testing/Production explicit capability ceilings (9/9 pure, 7/7 live), and symlink-safe core filesystem paths (6/6 pure, 2/2 integrated) are committed at `d398dbe`. Import 4/4, Phase 0 contract 37/37, exact evaluation 9/9, live secret references 5/5, trigger store 17/17, smoke 30/30, TypeScript, build, and isolated browser/Axe 90/90 passed. Sanitized public snapshot `b4c6888` is unauthenticated-readable. Host request `host-272201c8519779d4` reloaded the commit; health, hostile request checks, pure policy/filesystem checks, and smoke 30/30 passed from repository PID 20488.
...
- Safe default is scoped `standard` agent permissions. Full access must be explicit. Testing/Production deny capabilities absent from their exact reviewed workflow candidate, and inherited/node/plugin/custom-node denials compose without escalation. Imported executable packages install disabled and untrusted. Core run/artifact/AgentBrain paths reject traversal and symlinks. Generic HTTP/MCP credentials and provider API keys use macOS Keychain; only opaque names persist, and provider children receive a minimal environment.
