...
- Current focused evidence: schema 9/9, ports 5/5, comments 10/10, component manifests 10/10, scheduler 5/5, per-run resources 5/5, process-global model scheduling 8/8, reusable components 8/8, runtime recovery 36/36, MCP registry/client 12/12 plus live transports 4/4, Keychain secret registry/API/runtime/live 4/4 + 5/5 + 5/5 + 5/5, provider environment 1/1, durable trigger store 17/17, deterministic schedules 13/13, live trigger API 10/10, folder foundation 9/9, folder service 2/2, real folder two-restart recovery 5/5, security 4/4, and smoke 30/30. Trigger slices 5T-A through 5T-D and the first auth-reference slice are host-verified. The current LaunchAgent serves HTTP 200.
...
1. Finish the already-running Qwen3-Coder-Next A/B evaluation and retain the existing alias unless the candidate passes every promotion gate.
2. Continue Phase 5 with the remaining multi-trigger manager after the now-verified secure webhook 5T-D and Keychain auth-reference slices.
3. Add durable collaboration ownership leases/restart recovery before expanding Fable beyond one modifying worker.
4. Add Role Card persistence/runtime adapters and a migration-preview API/UI without modifying legacy agent data.
5. Keep production promotion untrusted until `docs/GOVERNANCE_HARDENING_PLAN.md` passes. Use Development for small projects.
