...
const component = (label, extension = {}) => markReusableComponent(migrateWorkflowDocument({ id: 'child', name: label, metadata: { extension }, nodes: [{ id: 'out', type: 'output', data: { label: 'Result' } }], edges: [], secretReferences: [{ id: 'never-return-this', purpose: 'generic' }], inputs: { private: true } }), { description: 'Reusable summary', sourceWorkflowId: 'parent' })
...
try {
  ...
  test('component metadata mutation rejects legacy Production records even without a lock flag', () => {
    const current = JSON.parse(fs.readFileSync(path.join(workflows, 'child.json'), 'utf8'))
    write({ ...current, environment: 'production', governance: { status: 'review-required', locked: false } })
    assert.throws(() => renameReusableComponent(workflows, versions, 'child', { name: 'Bypass' }), error => error.code === 'COMPONENT_LOCKED')
    write(current)
  })
...
console.log(`${passed}/9 reusable-component checks passed`)
