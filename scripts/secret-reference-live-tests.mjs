...
const workflowIds = [`secret-http-${suffix}`, `secret-mcp-${suffix}`], mcpId = `secret-mcp-${suffix}`
...
try {
  ...
  await test('production lock and node permissions prevent reference-bearing workflow changes or use', async () => {
    const promoted = await request(`/api/workflows/${workflowIds[0]}/promote`, { method: 'POST', ...json({ by: 'secret-reference-live-test', approval: 'fixture' }) }); if (!promoted.response.ok) throw new Error(JSON.stringify(promoted.body))
    const lockedSave = await request('/api/workflows', { method: 'POST', ...json({ ...promoted.body, secretReferences: [] }) }); if (lockedSave.response.status !== 423) throw new Error(`locked save returned ${lockedSave.response.status}`)
    const unlocked = await request(`/api/workflows/${workflowIds[0]}/unlock`, { method: 'POST', ...json({ reason: 'fixture permission check' }) }); if (!unlocked.response.ok) throw new Error(JSON.stringify(unlocked.body))
    const before = authorizedCalls, deniedWorkflow = { ...unlocked.body, nodes: unlocked.body.nodes.map(node => node.type === 'http' ? { ...node, permissions: { network: false } } : node) }
    ...
    const started = await request(`/api/workflows/${workflowIds[0]}/run`, { method: 'POST', ...json({ input: '' }) }); const run = await waitRun(started.body.runId)
