...
function repository() {
  ...
  write(root, '.gitignore', '.claude/worktrees/\nstate/collaboration/worktree-leases.json\n')
...
const packet = (taskId, overrides = {}) => ({
  ...
  readOnlyContextFiles: ['.gitignore'], dependencies: [], requiredTests: ['fixture'], permissionProfile: 'WORKTREE_IMPLEMENTATION',
...
await test('persists leases and blocks an active lease safely after manager restart', () => {
  const { root, baseSha } = repository()
  const first = createWorktreeManager({ repositoryRoot: root })
  const created = first.create({ taskId: 'restart-lease', baseSha })
  assert(fs.existsSync(first.leaseFile))
  const second = createWorktreeManager({ repositoryRoot: root })
  assert.deepEqual(second.recovery.blocked, ['restart-lease'])
  const recovered = second.get('restart-lease')
  assert.equal(recovered.state, 'BLOCKED')
  assert.equal(recovered.reason, 'ORPHANED_AFTER_RESTART')
  assert.equal(recovered.active, false)
  assert.equal(recovered.path, created.path)
  second.cleanup('restart-lease')
  fs.rmSync(root, { recursive: true, force: true })
})

await test('restores inactive leases but never adopts unknown worktrees', () => {
  const { root, baseSha } = repository()
  const first = createWorktreeManager({ repositoryRoot: root })
  first.create({ taskId: 'inactive-lease', baseSha }); first.markInactive('inactive-lease')
  const unknownPath = path.join(root, '.claude', 'worktrees', 'unknown')
  git(root, ['worktree', 'add', '-b', 'claude/unknown', unknownPath, baseSha])
  const second = createWorktreeManager({ repositoryRoot: root })
  assert.deepEqual(second.recovery.recovered, ['inactive-lease'])
  assert.equal(second.get('inactive-lease').state, 'INACTIVE')
  assert.equal(second.get('unknown'), null)
  second.cleanup('inactive-lease')
  git(root, ['worktree', 'remove', unknownPath]); git(root, ['branch', '-D', 'claude/unknown'])
  fs.rmSync(root, { recursive: true, force: true })
})

await test('fails closed on corrupt leases and preserves the worktree', () => {
  const { root, baseSha } = repository()
  const first = createWorktreeManager({ repositoryRoot: root })
  const created = first.create({ taskId: 'corrupt-lease', baseSha })
  fs.writeFileSync(first.leaseFile, '{bad-json')
  assert.throws(() => createWorktreeManager({ repositoryRoot: root }), error => error.code === 'COLLABORATION_LEASE_STORE_CORRUPT')
