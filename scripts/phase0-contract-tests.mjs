...
const promoted = await request('/api/workflows/phase0-custom-runtime/promote', { method: 'POST', ...json({ by: 'phase0-test', approval: 'fixture' }) })
check(promoted.response.ok && promoted.body.governance?.locked, 'production promotion locks workflow')
const lockedSave = await save({ ...promoted.body, name: 'Should Not Save' })
...
check(unlocked.response.ok && !unlocked.body.governance?.locked, 'audited unlock returns workflow to development')
...
check(resumed.status === 'done' && String(resumed.result).includes('repaired:preserved:resume-data'), 'run resumes from arbitrary node using preserved checkpoint', JSON.stringify(resumed))
...
check(retried.status === 'done' && retried.resumedFrom === failedRun.id, 'retry-branch endpoint creates a checkpoint-linked run')
