...
await test('canonical mutation rejects nonexistent, locked, and Production workflows', async () => {
  ...
  fs.writeFileSync(path.join(workflowDir, 'workflow-one.json'), JSON.stringify({ schemaVersion: 2, id: 'workflow-one', name: 'Legacy production', environment: 'production', nodes: [], edges: [], triggers: [], governance: { status: 'review-required', locked: false } }))
  await adapter.mutate('create', trigger()).then(() => { throw new Error('unlocked Production workflow accepted') }, error => { if (error.code !== 'WORKFLOW_LOCKED') throw error })
  fs.rmSync(dir, { recursive: true, force: true })
})
