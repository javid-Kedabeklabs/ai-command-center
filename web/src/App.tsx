...
function EvaluationLab() {
  ...
  </div><div className="section-title">Evaluation suites</div><div className="grid cols-2">{suites.map(suite => { const latest = suite.history?.[0]; const wrongWorkflow = !!selectedRunRecord && !!suite.workflowId && selectedRunRecord.workflowId !== suite.workflowId; return <div className="card eval-suite" key={suite.id}><div><b>{suite.name}</b><span className={latest?.passed ? 'ok' : latest ? 'bad' : ''}>{latest ? `${latest.score}% · ${latest.passed ? 'PASS' : 'FAIL'}` : 'Not run'}</span></div><small>{suite.workflowId || 'Any workflow'} · {suite.checks?.length || 0} checks · {suite.history?.length || 0} recorded runs</small>{latest && <><div className="eval-results">{latest.results?.map((result: any) => <span key={result.id}>{result.passed ? '✓' : '✗'} {result.detail}</span>)}</div><small className={latest.promotable ? 'ok' : 'bad'}>{latest.promotable ? `Promotable receipt · ${latest.candidateId} · ${latest.workflowVersion}` : `Diagnostic only · ${latest.nonPromotableReason || 'not candidate-bound'}`}</small></>}<div><button className="btn primary" disabled={busy === suite.id || wrongWorkflow || (!selectedRun && !checkValue)} title={wrongWorkflow ? 'Select a run from this suite’s workflow' : ''} onClick={() => runSuite(suite.id)}>Run evaluation</button><button className="btn" onClick={async () => { if (confirm(`Delete ${suite.name}?`)) { await api.deleteEvaluation(suite.id); refresh() } }}>Delete</button></div></div> })}</div>
