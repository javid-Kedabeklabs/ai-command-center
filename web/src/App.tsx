...
function CollaborationCenter() {
  ...
  return <div data-testid="collaboration-center">
    ...
    <div className="plain-callout" style={{ marginTop: 12 }}><b>Contract-complete packets: {status?.taskPacketContract.contractComplete ?? 0}</b><p>New Fable packets must freeze the reviewed Git base, exact contract-file hashes, acceptance-test commit, scenario IDs, changed-file budget, baseline estimate, and stop conditions before dispatch.</p><small>Packet schema v{status?.taskPacketContract.currentSchemaVersion ?? 2} · Legacy packets needing explicit upgrade: {status?.taskPacketContract.upgradeRequired ?? 0} · Legacy records remain readable but cannot be newly queued or dispatched</small></div>
