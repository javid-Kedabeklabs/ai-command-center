...
function CompanyWorld({ go }: { go: (page: Page) => void }) {
  const [world, setWorld] = useState<any>(null), [selectedAgent, setSelectedAgent] = useState(''), [task, setTask] = useState(''), [busy, setBusy] = useState(false)
...
function ExtensionCenter() {
  ...
  return <div className="extension-center"><p className="sub">Plugins and custom nodes are executable packages. Imported code stays disabled and untrusted until you review its provenance, dependencies, permissions, and tests.</p><div className="section-title">Plugin marketplace and installed plugins</div><div className="grid cols-2">{plugins.map(plugin => <div className="card extension-card" key={plugin.id}><div><b>{plugin.name}</b><span>v{plugin.version}</span></div><p>{plugin.description}</p><dl><dt>Publisher</dt><dd>{plugin.publisher || 'Unknown'}</dd><dt>Trust</dt><dd>{plugin.trustStatus || (plugin.verified ? 'verified' : 'not reviewed')}</dd><dt>Signature</dt><dd>{plugin.signatureStatus || (plugin.verified ? 'verified' : 'unsigned')}</dd><dt>Permissions</dt><dd>{plugin.permissions?.join(', ') || 'None declared'}</dd></dl><div className="extension-actions">{!plugin.installed ? <button className="btn primary" disabled={busy === plugin.id} onClick={() => act(plugin.id, () => api.installPlugin(plugin.id))}>Install verified package</button> : <>{plugin.trustStatus === 'untrusted' && <button className="btn" onClick={() => { if (confirm('Approve this plugin only after reviewing its source, permissions, dependencies, and tests?')) act(plugin.id, () => api.reviewPlugin(plugin.id)) }}>Review and trust</button>}<button className="btn" disabled={busy === plugin.id || plugin.trustStatus === 'untrusted'} onClick={() => act(plugin.id, () => api.togglePlugin(plugin.id, plugin.enabled === false))}>{plugin.enabled === false ? 'Enable' : 'Disable'}</button><button className="btn danger" onClick={() => { if (confirm(`Uninstall ${plugin.name} and its contributed nodes?`)) act(plugin.id, () => api.uninstallPlugin(plugin.id)) }}>Uninstall</button></>}</div></div>)}</div><div className="section-title">Installed custom nodes</div><div className="grid cols-2">{customNodes.map(node => <div className="card extension-card" key={node.id}><div><b>{node.icon || '◇'} {node.name}</b><span>v{node.version}</span></div><p>{node.description}</p><dl><dt>Adapter</dt><dd>{node.implementation?.kind}</dd><dt>Trust</dt><dd>{node.trustStatus}</dd><dt>Provenance</dt><dd>{node.provenance?.type || 'local'}</dd><dt>Tests</dt><dd>{node.tests?.length || 0}</dd></dl></div>)}</div></div>
}

/* ================= Evaluation Laboratory ================= */
function EvaluationLab() {
  const [suites, setSuites] = useState<any[]>([]), [workflows, setWorkflows] = useState<WfSummary[]>([]), [runs, setRuns] = useState<RunSummary[]>([]), [proposals, setProposals] = useState<any[]>([])
  const [name, setName] = useState('Quality gate'), [workflowId, setWorkflowId] = useState(''), [checkType, setCheckType] = useState('contains'), [checkValue, setCheckValue] = useState(''), [selectedRun, setSelectedRun] = useState(''), [busy, setBusy] = useState('')
  const refresh = useCallback(async () => { const [evaluationData, workflowData, runData, proposalData] = await Promise.all([api.evaluations(), api.workflows(), api.runs(), api.learningProposals()]); setSuites(evaluationData); setWorkflows(workflowData); setRuns(runData); setProposals(proposalData); if (!workflowId && workflowData[0]) setWorkflowId(workflowData[0].id) }, [workflowId])
  useEffect(() => { refresh().catch(() => {}) }, [refresh])
  const createSuite = async () => { if (!name.trim() || !checkValue.trim()) return; setBusy('create'); try { await api.saveEvaluation({ name, workflowId, checks: [{ id: 'primary', type: checkType, value: checkType === 'min-length' ? Number(checkValue) : checkValue }] }); setCheckValue(''); await refresh() } finally { setBusy('') } }
  const runSuite = async (id: string) => { setBusy(id); try { await api.runEvaluation(id, selectedRun ? { runId: selectedRun } : { output: checkValue }); await refresh() } finally { setBusy('') } }
  ...
  return <div className="evaluation-lab"><p className="sub">Deterministic checks are the default. Results attach to real runs and can block production promotion when configured as gates.</p><div className="grid cols-2">
    <div className="card"><div className="section-title" style={{ marginTop: 0 }}>Create deterministic suite</div><label>Name<input value={name} onChange={e => setName(e.target.value)} /></label><label>Workflow<select value={workflowId} onChange={e => setWorkflowId(e.target.value)}>{workflows.map(workflow => <option key={workflow.id} value={workflow.id}>{workflow.name}</option>)}</select></label><div className="eval-check-row"><select value={checkType} onChange={e => setCheckType(e.target.value)}><option value="contains">Contains</option><option value="not-contains">Does not contain</option><option value="min-length">Minimum length</option><option value="regex">Regular expression</option><option value="json">Valid JSON</option></select><input value={checkValue} onChange={e => setCheckValue(e.target.value)} placeholder="Expected value" /></div><button className="btn primary" disabled={busy === 'create'} onClick={createSuite}>Create suite</button></div>
    <div className="card"><div className="section-title" style={{ marginTop: 0 }}>Run evidence</div><label>Attach evaluation to a run<select value={selectedRun} onChange={e => setSelectedRun(e.target.value)}><option value="">Use typed sample value</option>{runs.slice(0, 100).map(run => <option key={run.id} value={run.id}>{run.title || run.task?.slice(0, 45) || run.id} · {run.status}</option>)}</select></label><p className="sub">A suite run against persisted output records its score, individual checks, run ID, and timestamp.</p><button className="btn" onClick={analyze} disabled={!workflowId || busy === 'analyze'}>Analyze failures for learning proposals</button></div>
  </div><div className="section-title">Evaluation suites</div><div className="grid cols-2">{suites.map(suite => { const latest = suite.history?.[0]; return <div className="card eval-suite" key={suite.id}><div><b>{suite.name}</b><span className={latest?.passed ? 'ok' : latest ? 'bad' : ''}>{latest ? `${latest.score}% · ${latest.passed ? 'PASS' : 'FAIL'}` : 'Not run'}</span></div><small>{suite.workflowId || 'Any workflow'} · {suite.checks?.length || 0} checks · {suite.history?.length || 0} recorded runs</small>{latest && <div className="eval-results">{latest.results?.map((result: any) => <span key={result.id}>{result.passed ? '✓' : '✗'} {result.detail}</span>)}</div>}<div><button className="btn primary" disabled={busy === suite.id} onClick={() => runSuite(suite.id)}>Run evaluation</button><button className="btn" onClick={async () => { if (confirm(`Delete ${suite.name}?`)) { await api.deleteEvaluation(suite.id); refresh() } }}>Delete</button></div></div> })}</div>
  <div className="section-title">Controlled learning proposals</div>{!proposals.length && <div className="empty">No evidence-backed proposals yet. Analyze a workflow after it has run.</div>}{proposals.map(proposal => <div className="card learning-proposal" key={proposal.id}><div><b>{proposal.title}</b><span>{proposal.status}</span></div><p>{proposal.rationale}</p><pre>{JSON.stringify(proposal.patch, null, 2)}</pre>{proposal.status === 'proposed' && <div><button className="btn primary" onClick={async () => { await api.decideLearningProposal(proposal.id, 'approved'); refresh() }}>Approve new version</button><button className="btn" onClick={async () => { await api.decideLearningProposal(proposal.id, 'rejected'); refresh() }}>Reject</button></div>}</div>)}</div>
}
