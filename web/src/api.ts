...
export const api = {
  ...
  plugins: (): Promise<any[]> => fetch('/api/plugins').then(j),
  ...
  reviewPlugin: (id: string): Promise<any> => fetch(`/api/plugins/${encodeURIComponent(id)}/review`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision: 'approve', by: 'owner' }) }).then(j),
  ...
  killAll: (): Promise<{ ok: boolean; stopped: number }> => fetch('/api/killall', { method: 'POST' }).then(j),
