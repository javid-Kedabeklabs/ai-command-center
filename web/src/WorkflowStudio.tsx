...
function WorkflowStudio({ mode, onModeChange }: { mode: Mode; onModeChange: (mode: Mode) => void }) {
  ...
  const copySelection = () => {
    const selectedIds = new Set(nodes.filter(node => node.selected || node.id === selectedNode).map(node => node.id))
    if (!selectedIds.size) { setNotice('Select one or more workflow steps to copy.'); return }
    clipboardRef.current = {
      nodes: nodes.filter(node => selectedIds.has(node.id)).map(node => ({ ...node, selected: false })),
      edges: edges.filter(edge => selectedIds.has(edge.source) && selectedIds.has(edge.target)).map(edge => ({ ...edge })),
    }
    setNotice(`Copied ${selectedIds.size} workflow item${selectedIds.size === 1 ? '' : 's'}.`)
  }

  const pasteSelection = () => {
    const copied = clipboardRef.current
    if (!copied?.nodes.length) { setNotice('Copy workflow steps before pasting.'); return }
    snapshot()
    const suffix = crypto.randomUUID().slice(0, 6), ids = new Map(copied.nodes.map(node => [node.id, `${node.id}-copy-${suffix}`]))
    const pasted = copied.nodes.map(node => ({ ...node, id: ids.get(node.id)!, parentId: node.parentId && ids.has(node.parentId) ? ids.get(node.parentId) : undefined, position: { x: node.position.x + 36, y: node.position.y + 36 }, selected: true, data: { ...node.data, label: `${String((node.data as any).label || 'Item')} copy`, status: '' } }))
    const pastedEdges = copied.edges.map(edge => ({ ...edge, id: `${edge.id}-copy-${suffix}`, source: ids.get(edge.source)!, target: ids.get(edge.target)!, selected: false }))
    setNodes(current => [...current.map(node => ({ ...node, selected: false })), ...pasted]); setEdges(current => [...current.map(edge => ({ ...edge, selected: false })), ...pastedEdges]); setSelectedNode(pasted[0].id); setSelectedEdge(''); setNotice(`Pasted ${pasted.length} workflow item${pasted.length === 1 ? '' : 's'} with internal connections.`)
  }
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      else if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'c' && !typing) { e.preventDefault(); copySelection() }
      else if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'v' && !typing) { e.preventDefault(); pasteSelection() }
      else if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'd' && !typing && selectedNode) { e.preventDefault(); duplicateNode(selectedNode) }
      ...
      else if (e.key === 'Escape' && !typing) { setNodes(current => current.map(node => node.selected ? { ...node, selected: false } : node)); setEdges(current => current.map(edge => edge.selected ? { ...edge, selected: false } : edge)); setSelectedNode(''); setSelectedEdge(''); setSelectedComment(''); setContextMenu(null) }
