...
test('AI Collaboration truthfully exposes durable review-gated worker state', async ({ page }) => {
  ...
  await expect(center).toContainText('Live product dispatch remains disabled')
  await expect(center).toContainText('Measured routing: shadow only')
  await expect(center).toContainText('Open self-challenge questions')
  await expect(center).toContainText('Q-0002 · Which project license should govern the public release?')
  const response = await page.request.get('/api/collaboration/status')
  expect(response.ok(), await response.text()).toBeTruthy()
  const status = await response.json()
  expect(status.dispatchEnabled).toBe(false)
  expect(status.dispatchContract).toMatchObject({ schemaVersion: 1, verified: true })
  expect(status.metrics).toMatchObject({ schemaVersion: 1, mode: 'SHADOW_ONLY', observations: 0 })
  ...
  const questionsResponse = await page.request.get('/api/collaboration/questions?status=OPEN')
  expect(questionsResponse.ok(), await questionsResponse.text()).toBeTruthy()
  const questions = await questionsResponse.json()
  expect(questions.questions.length).toBeGreaterThan(0)
  expect(questions.questions.every((item: { status: string }) => item.status === 'OPEN')).toBe(true)
