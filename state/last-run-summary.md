...
- Completed atomic accepted-delivery metrics for Fable and Qwen. Timing is derived from durable task/dispatch receipts; modifying outcomes require explicit integrated/rejected lease evidence; exact command replay is idempotent; escaped defects append separately; corrupt metrics disable only learning evidence without replacing the file or preventing product startup.
- Routing remains permanently shadow-only in this slice. Fable eligibility requires twelve accepted comparable observations, twelve baseline estimates, at least 80% first-pass acceptance, at least 15% lower median lead time, and zero boundary/S1/S2 regressions. Qwen eligibility requires twenty measured reviews, at least 25% accepted findings, and zero false blockers. Neither recommendation can dispatch, integrate, change policy, or weaken tests.
- The authoritative release verifier now passes 32/32 gates with twelve live browser journeys. Receipt: `fc5df2008429e49e7321bd27502418be848d82d52d3238d43811a1f66224a51d`.
- Committed accepted-delivery metrics at `274a0418fb779d5ac37806069001d9d2fdc969e4`. Reloaded through host request `host-8f0a2b9362fc321a`; health returned 200 after two attempts with receipt `40ea952ffdd6020a3da46dea4d2f6f5326279a4e30cec639657201bbecaec2f9`. Live status and metrics returned 200 with `SHADOW_ONLY`, zero observations, zero defects, insufficient Qwen evidence, and no automatic authority; dispatch/integration remain false and hostile Host returns 403.
...
- The previous dispatcher checkpoint passed 31/31 gates with twelve live browser journeys. Receipt: `0918dac2aca2f341475ee35acfb16bfdd7b6b8f5830de8705e644d3e3bd860b4`.
...
- Published sanitized source-only snapshot `1a7cb04368fc1359eecef3724c0acd0c3dd81292` on `review/current-20260713`. It contains no `data/` path, Gitleaks found zero findings across 2.28 MB, the push succeeded, and `git ls-remote` independently returned the same SHA.
