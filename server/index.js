...
fs.mkdirSync(OC_AGENT_DIR, { recursive: true })
...
// sync all on boot
for (const a of loadAgents()) writeOcAgent(a)
...
app.post('/api/workflows', (req, res) => {
  let w
  try { w = migrateWorkflowDocument(req.body) }
  catch (error) { return res.status(error?.code === 'FUTURE_WORKFLOW_SCHEMA' ? 409 : 400).json({ error: error.message }) }
  ...
  if (!w.id) {
    ...
    const existing = readJson(path.join(WF_DIR, w.id + '.json'), null)
    if (existing?.governance?.locked && !w._unlock) return res.status(423).json({ error: 'production workflow is locked; unlock it before editing' })
    if (existing) w = migrateWorkflowDocument(mergeWorkflowDocuments(existing, w))
  }
  delete w._unlock
  const serialized = JSON.stringify(w, null, 2)
  ...
  fs.writeFileSync(path.join(WF_DIR, w.id + '.json'), serialized)
  res.set('X-Workflow-Version', savedVersion.id).json(w)
})
app.post('/api/workflows/:id/promote', (req, res) => {
  if (!/^[a-z0-9_-]+$/i.test(req.params.id)) return res.status(400).json({ error: 'invalid workflow id' })
  const file = path.join(WF_DIR, req.params.id + '.json'), raw = readJson(file, null), w = raw ? migrateWorkflowDocument(raw) : null
  if (!w) return res.status(404).json({ error: 'workflow not found' })
  const errors = workflowErrors(w)
  if (errors.length) return res.status(400).json({ error: errors.join('; ') })
  const requiredEvaluations = Array.isArray(w.governance?.promotionGates?.evaluations) ? w.governance.promotionGates.evaluations.map(String) : Array.isArray(w.evaluations) ? w.evaluations.filter(item => typeof item === 'string') : []
  if (requiredEvaluations.length) {
    const suites = readJson(EVALUATIONS_FILE, []), failures = requiredEvaluations.map(id => ({ id, suite: suites.find(suite => suite.id === id) })).filter(item => !item.suite?.history?.[0]?.passed)
    if (failures.length) return res.status(409).json({ error: `production promotion blocked: passing evaluations required for ${failures.map(item => item.id).join(', ')}`, failedGates: failures.map(item => item.id) })
  }
  w.environment = 'production'
  w.governance = { ...(w.governance || {}), status: 'production', locked: true, promotedAt: Date.now(), promotedBy: String(req.body.by || 'owner').slice(0, 80), approval: String(req.body.approval || '').slice(0, 500) }
  for (const node of w.nodes) if (['python', 'shell', 'custom'].includes(node.type)) node.data = { ...(node.data || {}), codeMode: node.type === 'python' ? 'locked' : node.data?.codeMode, locked: true }
  atomicWriteJsonSync(file, w); appendAudit('workflow_promoted', { workflow: w.id, by: w.governance.promotedBy })
  res.json(w)
})
app.post('/api/workflows/:id/unlock', (req, res) => {
  ...
  const file = path.join(WF_DIR, req.params.id + '.json'), raw = readJson(file, null), w = raw ? migrateWorkflowDocument(raw) : null
  if (!w) return res.status(404).json({ error: 'workflow not found' })
  w.environment = 'development'; w.governance = { ...(w.governance || {}), status: 'draft', locked: false, unlockedAt: Date.now(), unlockReason: String(req.body.reason || 'Owner requested changes').slice(0, 500) }
  for (const node of w.nodes) if (node.data?.locked) node.data = { ...node.data, locked: false, codeMode: node.data.codeMode === 'locked' ? 'assisted' : node.data.codeMode }
  atomicWriteJsonSync(file, w); appendAudit('workflow_unlocked', { workflow: w.id, reason: w.governance.unlockReason })
  res.json(w)
})
...
app.post('/api/workflows/:id/versions/:versionId/restore', (req, res) => {
  ...
  try {
    ...
    const workflow = migrateWorkflowDocument({ ...version.workflow, id: req.params.id })
    ...
    res.json(workflow)
  } catch (error) { res.status(error?.code === 'MISSING_WORKFLOW_VERSION' ? 404 : 409).json({ error: error.message || 'version not found' }) }
})
...
app.post('/api/evaluations', (req, res) => {
  const list = readJson(EVALUATIONS_FILE, []), item = { id: safeSlug(req.body.id || req.body.name, `evaluation-${Date.now()}`), name: String(req.body.name || 'Evaluation').slice(0, 100), workflowId: String(req.body.workflowId || ''), checks: Array.isArray(req.body.checks) ? req.body.checks.slice(0, 50) : [], history: [], createdAt: Date.now() }
  const index = list.findIndex(x => x.id === item.id); if (index >= 0) item.history = list[index].history || [], list[index] = item; else list.push(item)
...
app.post('/api/evaluations/:id/run', (req, res) => {
  ...
  const value = String(req.body.output ?? run?.result ?? '')
  const results = (suite.checks || []).map((check, index) => {
    let passed = true, detail = ''
    if (check.type === 'contains') { passed = value.toLowerCase().includes(String(check.value || '').toLowerCase()); detail = `contains ${check.value}` }
    else if (check.type === 'not-contains') { passed = !value.toLowerCase().includes(String(check.value || '').toLowerCase()); detail = `does not contain ${check.value}` }
    else if (check.type === 'min-length') { passed = value.length >= Number(check.value || 0); detail = `${value.length} / ${check.value} characters` }
    else if (check.type === 'json') { try { JSON.parse(value) } catch { passed = false }; detail = 'valid JSON' }
    else if (check.type === 'regex') { try { passed = new RegExp(String(check.value || '')).test(value) } catch { passed = false }; detail = `matches ${check.value}` }
    return { id: check.id || `check-${index + 1}`, passed, detail }
  })
  const record = { id: crypto.randomUUID(), at: Date.now(), runId: req.body.runId || null, score: results.length ? Math.round(results.filter(x => x.passed).length / results.length * 100) : 100, passed: results.every(x => x.passed), results }
  suite.history = [record, ...(suite.history || [])].slice(0, 100); writeJson(EVALUATIONS_FILE, list); appendAudit('evaluation_run', { evaluation: suite.id, score: record.score })
  if (record.runId && run) {
    run.evaluations = [{ evaluationId: suite.id, name: suite.name, recordId: record.id, score: record.score, passed: record.passed, at: record.at }, ...(run.evaluations || [])].slice(0, 100)
...
app.post('/api/learning/proposals/:id/decision', (req, res) => {
  ...
  if (item.status === 'approved' && item.patch?.settings) { const file = path.join(WF_DIR, `${item.workflowId}.json`), wf = readJson(file, null); if (wf && !wf.governance?.locked) { wf.settings = { ...(wf.settings || {}), ...item.patch.settings }; atomicWriteJsonSync(file, wf); item.applied = true } }
...
app.post('/api/workflows/:id/run', async (req, res) => {
  ...
  const requestedVersion = String(req.body.workflowVersion || resumeSource?.workflowVersion || '').trim()
  ...
  try {
    ...
    wf = migrateWorkflowDocument(definition)
  } catch (error) {
  ...
  const run = { id: runId, type: 'workflow', workflowId: wf.id, workflowVersion: requestedVersion || null, workflowName: wf.name, task: input, status: 'running', dir: cwd, events: [], listeners: new Set(), approvals: {}, childRunIds: [], executionPolicy: policyEvidence, modelAbortController: new AbortController(), started: Date.now(), resumedFrom: resumeSource?.id || null, resumeFromNode: resumeFromNode || null, ...(triggerContext ? { triggerContext } : {}) }
  runs.set(runId, run)
...
app.post('/api/bundle/import', (req, res) => {
  ...
  try { for (const workflow of bundle.workflows || []) { const migrated = migrateWorkflowDocument(workflow); const errors = workflowErrors(migrated); if (errors.length) throw new Error(`${workflow.name || workflow.id}: ${errors.join('; ')}`) } }
...
app.post('/api/bundle/import/:id/decision', (req, res) => {
  ...
  try {
    for (const raw of bundle.agents || []) { const agent = { ...raw, permissions: req.body.preservePermissions === true ? raw.permissions || 'standard' : 'standard' }; const list = loadAgents(), index = list.findIndex(current => current.id === agent.id); if (index >= 0) list[index] = agent; else list.push(agent); saveAgents(list); writeOcAgent(agent); summary.agents++ }
    for (const raw of bundle.workflows || []) { const workflow = migrateWorkflowDocument(raw), file = path.join(WF_DIR, `${workflow.id}.json`); workflow.environment = 'development'; workflow.governance = { ...(workflow.governance || {}), status: 'imported-review', locked: false, importedAt: Date.now(), provenance: { type: 'bundle', proposalId: item.id } }; atomicWriteJsonSync(file, workflow); summary.workflows++ }
  ...
  } catch (error) { res.status(409).json({ error: error.message, summary }) }
})
