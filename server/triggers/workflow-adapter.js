...
export function createWorkflowTriggerAdapter({ workflowDir, versionDir }) {
  ...
  async function mutate(action, trigger) {
    ...
    if (workflow.governance?.locked) throw Object.assign(new Error('production workflow is locked; unlock it before changing triggers'), { code: 'WORKFLOW_LOCKED' })
  ...
  async function migrateLegacy(triggers) {
    ...
    for (const [workflowId, incoming] of byWorkflow) {
      ...
      const workflow = migrateWorkflowDocument(raw)
      if (workflow.governance?.locked) throw new Error(`legacy trigger migration requires unlocking workflow ${workflowId}`)
      let changed = false
